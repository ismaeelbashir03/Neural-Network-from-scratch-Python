...
class Model():
    ...
    # here we have our funtion for making new predictions on never seen before data
    def predict(self, X, *, batch_size = None):

        # we can defuakt our prediction steps
        pred_steps = 1

        # if we have batch size, we can calculate prediction steps
        if batch_size != None:
            pred_steps = len(X) // batch_size

            # we can also check if we rounded down and add one if so
            if batch_size * pred_steps < len(X):
                pred_steps += 1
        
        # here we initialise our output
        output = []

        # we can now loop through each batch and feed it forward through our network
        for step in range(pred_steps):
            
            # if we arnt using batches, put the whole input as one batch
            if batch_size == None:
                batch_x = X
            else:
                # if we are using batches, we slice our data
                batch_x = X[batch_size*step: (step+1)*batch_size]

            # we feed our batch through the network and indicate that we are not training
            # so we dont use the dropout layers if used
            batch_output = self.forward(batch_x, training = False)

            # we can now add our batch output to our whole output
            output.append(batch_output)

        # we can now return our output, but we want to make it a list of lists, 
        # not a list of lists of lists, so vstack, does this for us.
